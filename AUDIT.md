...
**All audit items resolved.** Follow-ups worth considering: real client/server network separation (client currently embeds the server), Testcontainers-based Postgres test for the Flyway migrations, and multi-replica support by moving hot game state to Redis (P1-7 note stands).
